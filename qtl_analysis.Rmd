...
scanone.addcov.both_interesting <- summary_scanone.addcov.both %>%
...
# Create dataframe for lodplots
scanone.addboth_lodplot_df <- lodplot_map_df(
  SCANONE = scanone.addboth.out.hk,
  SUMMARY = summary_scanone.addboth,
  PERMS = scanone.addboth.operm,
  alpha = 0.1,
  ci.funciton = "lodint"
  )
...
# lodplots for all the phenotypes
scanone.addboth_lodplot_all <- scanone.addboth_lodplot_df %>%
  group_by(phenotype) %>%
  nest %>%
  mutate(
    title = paste0(
...
load("output/qtl_objects/permutations/scanone.intcov.operm")
...
scanone.intboth.out.hk <- c(
  scanone.intcov.out.hk,
  scanone.intcov.out.hk - scanone.addboth.out.hk,
  labels = c("f", "i")
)
...
# transform the summary to tibble
summary_scanone.intcov.both <- summary.to.tibble(
  CROSS = ion_crossobject, 
...
# Export the summary as a csv
write_csv(
  summary_scanone.intcov.both,
  "output/tables/scanone_summary/summary_scanone.intcov.both.csv"
)
...
load("output/qtl_objects/permutations/scanone.WT.operm")
